...
export async function GET(request: Request) {
  ...
  try {
    ...
    /* v5.96: САМОЛЕЧЕНИЕ ПУСТОГО/БЕДНОГО КАБИНЕТА. Канал привязан, а постов
     * в ленте мало (< 30 — историю раньше никто не импортировал или импорт
     * оборвался по дедлайну: «0 постов», «Канал пуст», статистика из нулей).
     * Запускаем импорт истории из t.me/s после ответа (after) — один канал
     * за запрос, троттлинг 30 мин внутри backfill-модуля; за несколько
     * открытий кабинета история дозревает целиком. */
    const emptyChannel = result.find((c) => c.stats.posts < 30)
    if (emptyChannel) {
      after(() =>
