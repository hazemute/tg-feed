...
import { ChevronRight, History, Plus, Search, Trash2, TrendingUp, X, Check } from 'lucide-react'
 ...
 * каталог каналов с кнопками «+ Подписаться».
 */
export function SearchTab() {
  const { user, categories, setCategory, setTab, openChannel, searchSeed, clearSearchSeed } = useApp()
  ...
  /** Enter в поле (submit формы): сохраняем запрос всегда и ищем сразу */
  const submitSearch = () => {
    const term = q.trim()
    if (!term) return
    haptic('light')
  ...
  return (
    <div className="no-scrollbar mx-auto h-full w-full max-w-[680px] overflow-y-auto overscroll-contain pb-24">
      {/* Заголовок */}
      ...
      <div className="sticky top-0 z-10 bg-tg-bg px-4 pb-3">
        ...
        {/* Чипы-фильтры */}
        <div className="mt-4 flex gap-2.5" role="tablist" aria-label="Тип поиска">
          {FILTERS.map((f) => {
