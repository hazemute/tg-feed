...
export function MyChannelTab() {
  ...
  /* v5.80: ЖИВАЯ СТАТИСТИКА — тихий опрос каждые 20с, пока вкладка открыта и
     видима (просмотры/лайки/подписчики приезжают без перезагрузки экрана;
  ...
  const promotion = data?.promotion ?? { used: 0, limit: 7, available: false, credits: 0 }
...
/* ------------------------------------------------------------------ */
/* Привязка канала — через добавление бота (v5.80)                     */
/* ------------------------------------------------------------------ */
      ...
      {stage === 'input' ? (
        ...
        <>
          <div className="mt-4 px-5">
            ...
            {claimError && (
              <div className="mt-3 rounded-2xl bg-red-500/10 px-4 py-3 text-[13px] font-medium text-red-500">
                {claimError}
              </div>
            )}
          </div>
