...
import { CreditCard, Loader2, Rocket, Wallet } from 'lucide-react'
...
/**
 * Шит покупки пакета продвижений (v5.69) — докупка к бесплатному месячному
 * продвижению Snap Pro. Способ оплаты — переключатель из трёх вариантов:
 *  • «С баланса» — вся сумма с рублёвого кошелька, кредиты зачисляются сразу;
 *  • «50 / 50» — половина с баланса + счёт на половину картой;
 *  • «Картой» — счёт на всю сумму (ЮKassa, виджет прямо здесь).
 * Баланса нет → переключатель не показывается (только «Картой»).
 * Баланса меньше половины цены → «50 / 50» задизейблен с подсказкой.
 */
...
export function PromotePackSheet({
  ...
  // Открытие шита → свежие цена/баланс/способы (кэш не нужен — шит открывают редко)
  useEffect(() => {
    if (!open) return
    let alive = true
    setInfo(null)
    api<PackInfo>('/api/promote-pack')
      .then((d) => {
        ...
        setMethod(bal >= d.priceKop ? 'balance' : bal >= Math.ceil(d.priceKop / 2) ? 'half' : 'card')
      })
      .catch(() => {})
    return () => {
      alive = false
  ...
  const priceKop = info?.priceKop ?? 19_900
  const count = info?.count ?? 5
  ...
  return (
    <>
      <BottomSheet
        open={open}
        onClose={onClose}
        title="Пакет продвижений"
        subtitle="Докупка к бесплатному продвижению месяца"
      >
        {/* Пакет */}
        <div className="card-soft rounded-2xl bg-tg-surface p-4">
          <div className="flex items-center gap-3">
            <span className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-tg-link/10 text-tg-link" aria-hidden>
              <Rocket className="size-6" strokeWidth={1.8} />
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-[16px] font-bold text-tg-text">
                {count} продвижений · {fmtPrice(priceKop)}
              </p>
        ...
        {showSwitch ? (
          <div className="mt-3">
            <div className="flex rounded-xl bg-tg-sep/40 p-1" role="radiogroup" aria-label="Способ оплаты">
              {options.map((o) => (
                <button
                  key={o.id}
                  type="button"
                  ...
                  className={cn(
                    'flex h-11 flex-1 items-center justify-center gap-1.5 rounded-lg px-1 text-[12.5px] font-medium transition',
                    method === o.id ? 'bg-tg-bg font-semibold text-tg-text shadow-sm' : 'text-tg-hint',
