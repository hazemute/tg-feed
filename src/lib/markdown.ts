  ...
  return [
    {
      ...
      v: v
        .replace(/\*\*|__|~~|\|\||\^\^/g, '')
        // Битые обрывки премиум-эмодзи (обрезанное превью, легаси) не утекают
        // текстом в пост: «![e:ID](обрезанный-url», «![el:ID]» без (url),
        // обрыв внутри ID («![ev:5357…»). Полные маркеры сюда не попадают —
        // их SPAN_RE забирает раньше в emoji-спаны.
        .replace(/!\[e(?:v|l)?(?::\d+)?\]?\([^)\s]*$/g, '')
        .replace(/!\[e(?:v|l)?(?::\d+)?\](?!\()/g, '')
        .replace(/!\[e(?:v|l)?(?::\d+)?$/g, ''),
    },
