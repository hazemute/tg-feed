...
import { TIER_PRICES, AI_SEARCH_DAILY_LIMIT, PRO_PROMOTE_WEEKLY_LIMIT } from '@/lib/tiers'
...
function staticFacts(): string[] {
  ...
  return [
    ...
    `Snap Pro: ${rub(pro.monthKop)} ₽/мес или ${rub(pro.yearKop)} ₽/год (${pro.monthStars} / ${pro.yearStars} Stars) — всё из Plus + ИИ-контентщик для своего канала (анализ стиля → пост → картинка → публикация в TG), продвижение постов в ленту до ${PRO_PROMOTE_WEEKLY_LIMIT} раз в неделю, премиум-бейдж автора, CTA-кнопка.`,
